...
function recalculateIndicatorsAndCycles(candlesticks) {
    ...
    // Update Momentum Parameters (Fixed defaults, hidden from UI)
    const mCycs = 50;
    const mLbL = 5;
    const mLbR = 5;
    const mMin = 5;
    ...
    // Update Chart Settings for Projections
    if (chart) {
    ...
    // Read Sensitivity (Precision) from UI
    ...
    // We calculate it here.
    // Pass null for manualCycle, then sensitivity
    const invertedCyclesForTarget = cycleDetector.detectCycles(candlesticks, useMomentum, momentumValues, true /* invert */, minDuration, maxDuration, priorityMinDuration, null, sensitivity);
    ...
    if (showIndexCycles) {
        ...
        const cyclesToIndex = cycleDetector.detectCycles(candlesticks, useMomentum, momentumValues, true /* invert */, minDuration, maxDuration, priorityMinDuration, currentManualCycle, sensitivity);
    ...
    if (showInverseCycles) {
        ...
        const cyclesToInverse = cycleDetector.detectCycles(candlesticks, useMomentum, momentumValues, false /* invert */, minDuration, maxDuration, priorityMinDuration, currentManualCycle, sensitivity);
    ...
    updateStatistics(cycles, invertedCyclesForTarget, momentumValues, useMomentum);
    ...
    // showIndexCycles enables LONG orders (blue cycles)
    // showInverseCycles enables SHORT orders (red cycles)
    cycleBot.simulateLiveTrading(
        ...
        maxDuration,
        priorityMinDuration,
        showIndexCycles,    // enableLong - ordini LONG abilitati se cicli blu attivi
        showInverseCycles,  // enableShort - ordini SHORT abilitati se cicli rossi attivi
 ...
 */
function calculateCombinedRangeGains(candles, momentumValues, useMomentum) {
    const MIN_RANGE = 5;
    const MAX_RANGE = 55;
    const STEP = 2;
    const size = Math.ceil((MAX_RANGE - MIN_RANGE + 1) / STEP);
        ...
        for (let maxIdx = 0; maxIdx < size; maxIdx++) {
            ...
            try {
                ...
                testBot.simulateLiveTrading(
                    ...
                    maxDur,
                    true,
                    true,  // enableLong = true
                    true,  // enableShort = true (BOTH enabled!)
                    sensitivity
                );
...
function updateTradeStatsTable(trades) {
    ...
    // Total Loss (all negative PnL trades)
    ...
    // Gain BE - Positive PnL from Break Even exits
    const sumBE = (arr) => arr.filter(t => t.reason && t.reason.includes('Break_Even') && t.pnl > 0).reduce((s, t) => s + (t.pnl || 0), 0);
    const longGainBE = sumBE(longTrades);
    const shortGainBE = sumBE(shortTrades);
    setEl('stat-long-gain-be', '$' + longGainBE.toFixed(0));
    setEl('stat-short-gain-be', '$' + shortGainBE.toFixed(0));
    setEl('stat-total-gain-be', '$' + (longGainBE + shortGainBE).toFixed(0));
