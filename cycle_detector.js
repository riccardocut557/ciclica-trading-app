    ...
    findCycleEnd(candles, startIndex, useMomentum, momentumValues, invert, minDuration, maxDuration, priorityMinDuration = true, sensitivity = 1) {
        ...
        if (priorityMinDuration && minEndIndex <= maxEndIndex) {
            ...
            if (checkMin) {
                ...
                // SYMMETRIC RULE: Both cycle types check for confirming candle color
                if (invert) {
                    // Per cicli invertiti (Index/Blue): La chiusura deve essere una candela ROSSA
                    // (conferma che il prezzo sta scendendo dopo il massimo)
                    const isRed = candles[minEndIndex].close < candles[minEndIndex].open;
                    if (isRed) {
                        return this.buildCycle(candles, startIndex, checkMin.minIndex, minEndIndex, invert, firstValidEnd);
                    }
                    // Se non è rossa, continua la ricerca normale
                } else {
                    // Per cicli normali (Inverse/Red): La chiusura deve essere una candela VERDE
                    // (conferma che il prezzo sta salendo dopo il minimo)
