...
class CycleTradingBot {
                    ...
                    if (this.multiTradeEnabled && enableLong) {
                        ...
                        if (this.openPosition && this.openPosition.type === 'LONG' && lastCycle.startIndex === currentLongCycleStart) {
                            if (meetsAdvancedFilters) {
                                ...
                                // Store cycle end candle data for pyramiding
                                if (this.openPosition) {
                                    this.openPosition.cycleEndLow = liveCandles[lastCycle.endIndex].low;
                                    this.openPosition.cycleEndClose = liveCandles[lastCycle.endIndex].close;
                                }
                                this.equityCurve.push({ index: i, balance: this.currentBalance });
                            }
                        }
                    ...
                    if (this.multiTradeEnabled && enableShort) {
                        ...
                        if (this.openPosition && this.openPosition.type === 'SHORT' && lastCycle.startIndex === currentShortCycleStart) {
                            if (meetsAdvancedFilters) {
                                ...
                                // Store cycle end candle data for pyramiding
                                if (this.openPosition) {
                                    this.openPosition.cycleEndHigh = liveCandles[lastCycle.endIndex].high;
                                    this.openPosition.cycleEndClose = liveCandles[lastCycle.endIndex].close;
                                }
                                this.equityCurve.push({ index: i, balance: this.currentBalance });
                            }
            ...
            if (canConfirmPending) {
                ...
                if (barConfirmed) {
                    ...
                    if (pendingConfirmBars >= 2) {
                        ...
                        // Store cycle end candle data for pyramiding
                        if (this.openPosition && pendingSignal.cycle) {
                            if (pendingSignal.type === 'SHORT') {
                                this.openPosition.cycleEndHigh = liveCandles[pendingSignal.cycle.endIndex].high;
                                this.openPosition.cycleEndClose = liveCandles[pendingSignal.cycle.endIndex].close;
                            } else if (pendingSignal.type === 'LONG') {
                                this.openPosition.cycleEndLow = liveCandles[pendingSignal.cycle.endIndex].low;
                                this.openPosition.cycleEndClose = liveCandles[pendingSignal.cycle.endIndex].close;
                            }
                        }
                        this.equityCurve.push({ index: i, balance: this.currentBalance });
    ...
    openTrade(type, price, index, slPrice, counterTrend = false, cycle = null, atrValue = 0) {
        ...
        this.openPosition = {
            ...
            lowestPrice: price,
            // For pyramiding: store cycle end candle's high, low and close
            cycleEndHigh: null,
            cycleEndLow: null,
            cycleEndClose: null,
            pyramidCount: 0 // Track how many times we've added to position
        };
    }
    ...
    checkAdvancedExit(candle, index) {
        ...
        if (pos.type === 'LONG') {
            ...
            // Break-even SL check
            // Trigger if LOW touches Entry (or lower) - MUST be checked BEFORE TP1 return
            ...
            if (!pos.partialClosed && candle.high >= tp1Price && tp1Price > pos.entryPrice) {
                ...
                // Check BE immediately on the same candle (if price wicked back to entry)
            ...
            // 4. LONG Pyramiding: Add 50% when bar goes below cycle low but closes above cycle close
            // This is a support pattern - price dips below support but gets bought back up
            if (this.pyramidingEnabled && pos.cycleEndLow && pos.cycleEndClose && pos.pyramidCount < 3) {
                // Condition: Low goes below cycle-end low AND Close is above cycle-end close
                if (candle.low < pos.cycleEndLow && candle.close > pos.cycleEndClose) {
                    this.addToPosition(candle.close, index, 0.5); // Add 50%
                    pos.pyramidCount++;
                    console.log(`[Pyramid LONG] Added 50% at bar ${index}, total pyramids: ${pos.pyramidCount}`);
                }
            }
        }
        // === SHORT EXIT ===
        if (pos.type === 'SHORT') {
            ...
            // 4. SHORT Pyramiding: Add 50% when bar exceeds cycle high but closes below cycle close
            // This is a rejection pattern - price spikes above resistance but gets rejected
            if (this.pyramidingEnabled && pos.cycleEndHigh && pos.cycleEndClose && pos.pyramidCount < 3) {
                // Condition: High exceeds cycle-end high AND Close is below cycle-end close
                if (candle.high > pos.cycleEndHigh && candle.close < pos.cycleEndClose) {
                    this.addToPosition(candle.close, index, 0.5); // Add 50%
                    pos.pyramidCount++;
                    console.log(`[Pyramid SHORT] Added 50% at bar ${index}, total pyramids: ${pos.pyramidCount}`);
                }
            }
        }
    ...
    addToPosition(price, index, fraction) {
        if (!this.openPosition) return;
        const pos = this.openPosition;

        // Calculate additional capital as fraction of current balance
        const additionalCapital = this.currentBalance * (this.capitalPercentage / 100) * fraction;
        const additionalSize = (additionalCapital * this.leverage) / price;

        // Update position with weighted average entry price
        const totalSize = pos.positionSize + additionalSize;
        const totalCapital = pos.capitalUsed + additionalCapital;

        // Weighted average entry price
        pos.entryPrice = ((pos.entryPrice * pos.positionSize) + (price * additionalSize)) / totalSize;
        pos.positionSize = totalSize;
        pos.capitalUsed = totalCapital;
        pos.initialCapital = totalCapital;
        pos.initialSize = totalSize;
    }
